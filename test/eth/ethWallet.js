...
describe('Ethereum Wallet API:', function() {
  ...
  describe('Invite non BitGo user', function() {
    ...
    it('arguments', function(done) {
      assert.throws(function() { bitgo.ethWallets().cancelInvite({}, function() {}); });
  ...
  describe('Share wallet', function() {
    ...
    before(function() {
      return bitgo.ethWallets().listShares({})
      .then(function(result){
        var cancels = result.outgoing.map(function(share) {
          return bitgo.ethWallets().cancelShare({ walletShareId: share.id });
        });
    ...
    it('share a wallet and then cancel the share', function(done) {
      ...
      .then(function(result){
        ...
        return bitgo.ethWallets().cancelShare({ walletShareId: cancelledWalletShareId }, function(err, result) {
  ...
  describe('Get wallet share list', function() {
    ...
    it('wallet share should be in receiver list', function(done) {
      bitgoSharedKeyUser.ethWallets().listShares({})
      .then(function(result){
  ...
  describe('Accept wallet share', function (){
    ...
    it('accept a wallet share with only view permissions', function(done) {
      bitgoSharedKeyUser.ethWallets().acceptShare({walletShareId: walletShareIdWithViewPermissions})
      .then(function(result) {
        result.should.have.property('state');
        result.should.have.property('changed');
        ...
        return bitgoSharedKeyUser.ethWallets().listShares({})
      })
      .then(function(result) {
    ...
    it('accept a wallet share with spend permissions', function(done) {
      ...
      .then(function() {
        return bitgoSharedKeyUser.ethWallets().acceptShare({walletShareId: walletShareIdWithSpendPermissions, userPassword: TestBitGo.TEST_SHARED_KEY_PASSWORD})
        .then(function (result) {
          result.should.have.property('state');
          result.should.have.property('changed');
          result.state.should.equal('accepted');
  ...
  describe('Wallet shares with skip keychain', function () {
    ...
    it('accept a wallet share without password', function(done) {
      bitgoSharedKeyUser.unlock({'otp': '0000000'})
      .then(function() {
        return bitgoSharedKeyUser.ethWallets().acceptShare({ walletShareId: walletShareId, overrideEncryptedXprv: 'test' })
        .then(function (result) {
          result.should.have.property('state');
          result.should.have.property('changed');
          result.state.should.equal('accepted');
  ...
  describe('Unspents', function() {
    ...
    before(function() {
      ...
      .then(function() {
        return consolidationBitgo.ethWallets().get({id: TestBitGo.TEST_WALLET2_ADDRESS})
      })
      .then(function(result) {
        sharedWallet = result;
      });
